...
Project config precedence: `config/organization.yaml` < optional `config/user.yaml` < project YAML. Custom Skills are discovered under `skills/custom/` without executing arbitrary code. Template packages require compatible design tokens/layout registry and a real local master; adding a registry entry alone does not validate a new template.
...
Checkpoints for `research_plan`, `storyline`, `final_preview` may be set in project YAML. `ppt approve` only records an explicit local acknowledgement. Automatic repair currently proposes fixes but does not loop through arbitrary repair attempts. Local logs and project manifests are not uploaded. Future external AI provider calls require separate privacy/provider policy.

Test commands: `npm test`; `npm run validate`; `./CPGHBPPT doctor`. The public Renderer is `python-pptx`; proprietary Artifact Tool code and dependencies are not included. Golden snapshots are environment-sensitive and must be reviewed when fonts/template change.
