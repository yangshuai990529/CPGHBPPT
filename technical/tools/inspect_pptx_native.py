...
with zipfile.ZipFile(file) as z:
 ...
 pics=shapes=charts=0
 for n in slides:
  root=ET.fromstring(z.read(n))
  pics+=sum(x.tag.endswith('}pic') for x in root.iter());shapes+=sum(x.tag.endswith('}sp') for x in root.iter());charts+=sum(x.tag.endswith('}graphicFrame') for x in root.iter())
 ...
 print(json.dumps({'status':'pass','slide_count':len(slides),'native_images':pics,'editable_shapes':shapes,'graphic_frames':charts,'media_files':sum(n.startswith('ppt/media/') for n in names)}))
