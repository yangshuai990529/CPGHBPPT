...
export async function runResearch({brief,deckPlan,outputDir,browser=new HttpBrowserProvider(),search,assets=[],cacheDir,now=()=>new Date()}) {
  ...
  const ranked=rankSources(candidates.map(x=>({...x,authority_level:/^[A-E]$/.test(x.authority_level??'')?x.authority_level:authority(x.url)})),'product');
