...
export function runRuleBasedQa(models, slideSpecs, assetList=[]) {
  ...
  models.forEach((model, index) => {
    ...
    if(spec.visual_requirement?.entities?.length){if(!renderedImages.length)add(issues,index,'REQUIRED_VISUAL_MISSING','error','Slide requested webpage screenshots but none were rendered');for(const entity of spec.visual_requirement.entities)if(!(spec.assets??[]).some(a=>a.status==='approved'&&a.entity===entity))add(issues,index,'VISUAL_ENTITY_MISSING','warning',`No verified screenshot available for ${entity}`);}
