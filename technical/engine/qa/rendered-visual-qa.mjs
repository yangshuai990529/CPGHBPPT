...
export async function inspectRenderedSlides(images,models){
  ...
  for(let i=0;i<images.length;i++){
    ...
    const hasVisual=model.elements.some(x=>['Image','Chart'].includes(x.type)),textChars=model.elements.filter(x=>x.text&&x.type!=='Source').reduce((n,x)=>n+x.text.length,0),activeRows=new Set(active.map(x=>x.row)).size,activeCols=new Set(active.map(x=>x.col)).size;
